...
class OpenSettingsSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: OpenSettings?, messageChannelSuffix: String = "") {
    let channelSuffix = messageChannelSuffix.count > 0 ? ".\(messageChannelSuffix)" : ""
    /// https://developer.apple.com/documentation/uikit/uiapplication/1623042-opensettingsurlstring
    let openSettingsChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.listen_to_music_by_location.OpenSettings.openSettings\(channelSuffix)", binaryMessenger: binaryMessenger)
    if let api = api {
